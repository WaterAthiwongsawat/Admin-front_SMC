...
export default {
    ...
    methods: {
        async initialize() {
            ...
            try {
                const data = await this.axios.get('http://localhost:9001/students')
        ...
        async deleteItemConfirm() {
            try {
                const response = await this.axios.delete('http://localhost:9001/student/' + this.idforDelete)
                this.initialize()
            } catch (error) {
    ...
    if (action === 'เพิ่มข้อมูล') {
        ...
        try {
            var dataResponse = await this.axios.put('http://localhost:9001/student/' + this.idstudent, data);
